...
class SEBICompliance:
    ...
    def generate_daily_report(self, date_str: str | None = None) -> dict:
        ...
        Returns a dict with summary stats and full audit log for the day.
        """
        import datetime
        if date_str is None:
        ...
        with self._lock:
            # Filter audit log for today's entries
            today_records = self._audit_log.get(date_str, [])
            today_entries = [r.__dict__ for r in today_records]

        # Count by event type (decision field used as event type)
        ...
        return {
            ...
            "registered_algos": list(APPROVED_ALGO_IDS.keys()),
            "audit_entries": today_entries[:200],  # cap at 200 for API response
